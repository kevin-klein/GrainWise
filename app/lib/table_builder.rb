class TableBuilder
  ...
  def build(grains)
    attributes = ["Feature",	"Sample", "Species", "Length 1 (mm)", "Length 2 (mm)", "Average length", "Breadth 1 (mm)", "Breadth 2 (mm)", "Average breadth", "Thickness (mm)"]
    ...
    grains.each do |grain|
      ...
      csv << [
        ...
        grain.identifier,
        grain.strain.name,
        number_without_unit(dorsal_length), # length 1
        number_without_unit(ventral_length), # length 2
        number_without_unit({unit: "mm", value: average_length}),
        number_without_unit(dorsal_width), # breadth 1
        number_without_unit(ventral_width), # breadth 2
        number_without_unit({unit: "mm", value: average_breadth}), # average breadth
        number_without_unit(thickness), # thickness
      ]
    end
